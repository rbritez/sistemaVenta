...
function addSociete( $nom, $adresse,$logo,$ext_logo )
{
	...
	$this->Image($logo , 5 ,-14, 60 , 60 , $ext_logo);
	$this->SetXY( $x1, $y1 );
...
function fact_dev( $libelle)
{
    ...
    while ( $loop == 0 )
    {
       ...
       $sz = $this->GetStringWidth( $tipoFactura );
       if ( ($r1+$sz) > $r2 )
          $szfont --;
       else
          $loop ++;
	...
	$this->Cell($r2-$r1 -1,5, $tipoFactura, 0, 0, "C" );
...
function favser($num){
    ...
    $this->RoundedRect($r1, $y1, ($r2 - $r1), $y2, 2.5, 'DF');
    $this->SetXY( $r1+1, $y1+2);
	$this->Cell($r2-$r1 -1,5, $sercod, 0, 0, "C" );
}
...
// Client address
function addClientAdresse( $cliente,$domicilio,$num_documento,$telefono )
{
	$r1     = $this->w - 190;
	$r2     = $r1 + 68;
	$y1     = 50;
